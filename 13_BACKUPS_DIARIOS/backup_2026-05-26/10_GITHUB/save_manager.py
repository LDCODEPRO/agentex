...
class SaveManager:
    ...
    def run_full_backup(self) -> dict:
        ...
        print(f"      {'✅' if results['local']['ok'] else '❌'} {results['local']['message']}")

        print("[2/3] 🐙 GitHub commit + push...")
        results["github"] = self._backup_github()
        print(f"      {'✅' if results['github']['ok'] else '❌'} {results['github']['message']}")

        print("[3/3] 📓 Obsidian export...")
        results["obsidian"] = self._backup_obsidian()
        print(f"      {'✅' if results['obsidian']['ok'] else '❌'} {results['obsidian']['message']}")

        # Registrar no banco
        self._log_backup(results)

        print(f"\n[SaveManager] Backup concluído — {datetime.now().strftime('%H:%M:%S')}")
        return results
